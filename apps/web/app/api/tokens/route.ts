...
const REDIRECT_URI = `${process.env.NEXT_PUBLIC_SITE_URL}/api/tokens`
...
async function requireUserScopes(
	accessToken: string,
	...
	const authorizedScopes = data.scope?.split(' ') || []

	if (requiredScopes.some((scope) => !authorizedScopes.includes(scope)))
